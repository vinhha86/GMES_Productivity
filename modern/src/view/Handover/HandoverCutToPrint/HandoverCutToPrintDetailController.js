Ext.define('GSmartApp.view.handover.HandoverCutToPrintDetailController', {
    ...
    init: function() {
        ...
        viewModel.set('viewId', 'handover_cut_toprint_edit');
        ...
        this.loadStoreFromInfo(orgtypestring_from, session.user);
    ...
    loadStoreFromInfo: function(orgtypestring_from, userid_link){
        var viewModel = this.getViewModel();
        var ListOrgStore_From = viewModel.getStore('ListOrgStore_From');
    ...
    onOrgToComboSelect: function(cbbox, newValue, oldValue, eOpts){
        ...
        if(id == 0 || id == null){
            if(viewId == 'handover_cut_toprint_edit' && newValue != null){
            ...
            function (success, response, options) {
                ...
                if (success) {
                    if(response.message == 'Phiếu đã được bên nhận xác nhận'){
                        ...
                        m.redirectTo("handover_cut_toprint");
                    }
                } else {
