Ext.define('GSmartApp.view.handover.HandoverPackFromLineDetailController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.HandoverPackFromLineDetailController',
    init: function() {
        var session = GSmartApp.util.State.get('session'); // console.log(session);
        var viewModel = this.getViewModel();
        viewModel.set('viewId', 'handover_pack_fromline_edit');
    ...
    loadUserInfo: function(userid_link){
        ...
        var mainView = Ext.getCmp('handover_pack_fromline_edit');
        if(mainView) {
            mainView.setMasked({
                xtype: 'loadmask',
                message: 'Đang tải'
            ...
            function (success, response, options) {
                ...
                if (success) {
                    ...
                    if(data.orgid_link == 1){
                        ...
                        ListOrgStore_To.getbyParentandType(data.orgid_link, orgtypestring_to);
                    }
                }else{
            ...
            function (success, response, options) {
                ...
                if (success) {
                    if (response.respcode == 200) {
                        ...
                        m.redirectTo("handover_pack_fromline" + "/" + response.data.id + "/edit");
                    }
                    else {
    ...
    onBtnSearch:function(){
        ...
        dialog.down('#HandoverDetailPorderSearch').getController().on('selectPOrder', function (record) {
            ...
            if(id == 0 || id == null){
                if(viewId == 'handover_pack_fromline_edit'){
    ...
    onOrgToComboSelect: function(cbbox, newValue, oldValue, eOpts){
        ...
        var id = viewModel.get('currentRec.id');
        if(id == 0 || id == null){
            if(viewId == 'handover_pack_fromline_edit' && newValue != null){
                var porderid_link = viewModel.get('currentRec.porderid_link');
                m.loadHandoverProductOnPorderSelect(porderid_link);
            }
        }
            ...
            function (success, response, options) {
                ...
                if (success) {
                    if(response.message == 'Phiếu đã được bên nhận xác nhận'){
                        ...
                        m.redirectTo("handover_pack_fromline");
                    }
                } else {
