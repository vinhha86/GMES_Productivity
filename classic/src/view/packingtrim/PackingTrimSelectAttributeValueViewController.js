Ext.define('GSmartApp.view.packingtrim.PackingTrimSelectAttributeValueViewController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.PackingTrimSelectAttributeValueViewController',
    init: function () {
        var me = this.getView();
            ...
            function (success, response, options) {
                if (success) {
                    ...
                    if (response.respcode == 200) {
                        mainView = Ext.getCmp('PackingTrimAttributeView');
                        mainView.getStore().load();

                        SKUView = Ext.getCmp('PackingTrimSKUView');
    ...
    onLuu: function () {
        ...
        if (me.IdAttribute == 35 || me.IdAttribute == 36) {
            Ext.Msg.show({
                title: 'Thông báo',
                msg: 'Thay đổi giá trị thuộc tính của màu , cỡ phụ liệu sẽ thay đổi đến mã SKU của phụ liệu may',
    ...
    loadAttributeValueStore: function () {
        ...
        params.attributeid_link = me.IdAttribute;
        params.productid_link = me.IdProduct;
        GSmartApp.Ajax.post('/gsmartinv/api/v1/productattribute/getvalue', Ext.JSON.encode(params),
            function (success, response, options) {
                if (success) {
                    var response = Ext.decode(response.responseText);
                    if (response.respcode == 200) {
